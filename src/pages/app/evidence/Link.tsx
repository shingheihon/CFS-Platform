...
export default function LinkEvidence() {
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-slate-900 mb-2">Link Evidence to Incident</h1>
        <p className="text-slate-600">Associate evidence items with cases and incidents</p>
      </div>

      <div className="bg-white border border-slate-200 rounded-lg p-6 shadow-sm">
        <h2 className="text-xl font-bold text-slate-900 mb-4">Link Evidence</h2>
        <form className="space-y-6">
          ...
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-2">Relationship Description</label>
            <textarea
              rows={3}
      ...
      <div className="bg-white border border-slate-200 rounded-lg p-6 shadow-sm">
        <h2 className="text-xl font-bold text-slate-900 mb-4">Existing Evidence Links</h2>
          ...
          {[
            ...
            <div key={index} className="bg-slate-50 border border-slate-200 rounded-lg p-4">
              <div className="flex items-start justify-between mb-2">
                <div>
                  <div className="text-slate-900 font-semibold mb-1">{link.evidence} → {link.linked}</div>
                  <div className="text-sm text-slate-600">{link.type}</div>
                </div>
                <span className="px-3 py-1 bg-green-100 text-green-700 rounded text-xs font-semibold">
                  Linked
                </span>
              </div>
              <p className="text-sm text-slate-600">{link.relation}</p>
            </div>
          ))}
        </div>
      </div>
